...
pkg_bld_dir="$OUT_DIR/$pkg_src_dir-shared"
METIS_DIR="$pkg_bld_dir"
pkg="METIS shared v$METIS_FULL_VERSION"
pkg_version="shared $METIS_FULL_VERSION"
...
function metis_build()
{
   if package_build_is_good; then
      ...
      return 0
       ...
       cd "$OUT_DIR" && tar zxf "$METIS_SOURCE_FILE" && \
           mv "$OUT_DIR/$pkg_src_dir" "$pkg_bld_dir" || {
   ...
   echo "Building $pkg, sending output to ${pkg_bld_dir}_build.log ..." && {
      if [[ "$METIS_VERSION" = "4" ]]; then
         ...
         make -C Lib CC="$MPICC" OPTFLAGS="$CFLAGS" \
              COPTIONS=-fPIC LDOPTIONS=-shared -j $num_proc_build && \
         mv libmetis.a libmetis.so
      else
...
{
   metis_download && metis_build && ln -sf $METIS_DIR $OUT_DIR/metis-4.0
}
