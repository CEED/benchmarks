...
pkg_bld_dir="$OUT_DIR/mfem-okina"
MFEM_DIR="$pkg_bld_dir"
# The mfem_branch can be set at the command line of the go.sh call
mfem_branch="${mfem_branch:-okina-wip}"
MFEM_BRANCH="${mfem_branch}"
pkg_var_prefix="mfem_okina_"
pkg="MFEM-okina (branch ${mfem_branch})"
...
function mfem_branch_build()
{
   ...
   local nvcc_flags= cuda_libs=
   nvcc_flags="--restrict --expt-extended-lambda -x=cu -arch=sm_60"
   nvcc_flags+=" $CUFLAGS -std=c++11 -m64"
   nvcc_flags+=" -I. -I$cuda_home/samples/common/inc"
   cuda_libs="-lrt -lcuda -lcudart -lcudadevrt -lnvToolsExt"
   ...
   print_variables "$pkg_var_prefix" \
      HYPRE_DIR METIS_DIR METIS_VERSION SUNDIALS_DIR MFEM_BRANCH \
