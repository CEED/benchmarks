...
for plt in pl_set:
   ...
   for np in np_set:
      ...
      d=asarray(sorted(d))
      tpn=np/num_nodes
      plot(d[:,0],d[:,1],'o-',color=colors[i%cm_size],
           label='%s, %i t/n, q=p+%i'%(compilers[0],tpn,rel_qpts[0]))
      if len(compilers)==2:
         plot(d[:,0],d[:,2],'d-',color=colors[i%cm_size],
              label='%s, %i t/n, q=p+%i'%(compilers[-1],tpn,rel_qpts[0]))
         fill_between(d[:,0],d[:,1],d[:,2],facecolor=colors[i%cm_size],alpha=0.1)
      i=i+1
   i=0
   for np in np_set:
      rel_qpts=[int(run['quadrature-pts']**(1./3)+0.5)-run['order']
      ...
      d=asarray(sorted(d))
      tpn=np/num_nodes
      plot(d[:,0],d[:,1],'s--',color=colors[i%cm_size],
           label='%s, %i t/n, q=p+%i'%(compilers[0],tpn,rel_qpts[1]))
      if len(compilers)==2:
         plot(d[:,0],d[:,2],'^--',color=colors[i%cm_size],
   ...
   if 1: # write .pdf file?
      if len(compilers)==1:
         pdf_file='plot1_%s_%s_%s_%s_N%03i.pdf'%(
                  code,test_short,config_short,compilers[0],num_nodes)
      else:
         pdf_file='plot1_%s_%s_%s_N%03i.pdf'%(
