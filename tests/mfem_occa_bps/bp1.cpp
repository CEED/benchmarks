...
//
// Compile with: make bp1
//
// Sample runs:  ./bp1_occa.sh
//               ./dp1_acro.sh
//
...
int main(int argc, char *argv[])
{
  ...
  if (use_acrotensor) {
    a->AddDomainIntegrator(new AcroMassIntegrator(1.0));
     ...
     OccaIntegrator *mi = new OccaMassIntegrator(1.0, ir);
     a->AddDomainIntegrator(mi);

     const int points = mi->GetIntegrationRule().GetNPoints();
