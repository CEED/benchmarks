...
int main(int argc, char *argv[])
{
   ...
   ParBilinearForm vd(&fes);
   vd.SetAssemblyLevel(AssemblyLevel::PARTIAL);
   vd.AddDomainIntegrator(new VectorDiffusionIntegrator);
   vd.Assemble();
   x.ParallelProject(x_tdofs);
   vd.Mult(x_tdofs, y_tdofs);
   y.Distribute(y_tdofs);

   // **************************************************************************
#if 0
   ParBilinearForm vmass_fa(&fes);
   vmass_fa.AddDomainIntegrator(new VectorDiffusionIntegrator);
   vmass_fa.Assemble();
   vmass_fa.Finalize();
   x.ParallelProject(x_tdofs);
   ...
   std::cout << "||A*x_{pa} - A*x_{fa}|| = " << error << std::endl;
#endif
