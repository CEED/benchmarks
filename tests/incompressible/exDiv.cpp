...
// Prescribed periodic velocity
void u_periodic(const Vector &x, Vector &u)
{
   //u(0) = -cos(4.0*M_PI * x(0)) * sin(3.0*M_PI * x(1));
   //u(1) = 14*sin(2.0*M_PI * x(0)) * cos(5.0*M_PI * x(1));
   //u(2) = 0.0;
   //u(0) =    cos(2*M_PI * x(0)) * sin(2*M_PI * x(1));
   //u(1) =    cos(2*M_PI * x(0)) * sin(2*M_PI * x(1));
   //u(2) =    cos(2*M_PI * x(0)) * sin(2*M_PI * x(1));
   u(0) =    cos(2*M_PI * x(0)) * sin(2*M_PI * x(1));
   u(1) = -2*cos(2*M_PI * x(0)) * sin(2*M_PI * x(1));
   if (u.Size() == 3)
   {
      u(2) = x(0) + x(1) + x(2);
   }
...
int main(int argc, char *argv[])
{
   ...
   HYPRE_Int size = vel_size + pres_size;
   if (myid == 0)
   {
   ...
   if (nattr >= 1)
   {
      ess_bdr = 0;
      ess_bdr[0] = 1;
      vel_fes.GetEssentialTrueDofs(ess_bdr, ess_trial_tdof_list);
   }
   if (nattr >= 2)
   {
      ess_bdr = 0;
      ess_bdr[1] = 1;
      pres_fes.GetEssentialTrueDofs(ess_bdr, ess_test_tdof_list);
   }

   // **************************************************************************
   ConstantCoefficient one{1.0};
   VectorFunctionCoefficient ucoeff{dim, u_periodic};
   ...
   if (transpose)
   {
      vel.Randomize(3);
      //vel = 1.0;
      pressure.ProjectCoefficient(pcoeff);
   }
   else
   {
      vel.ProjectCoefficient(ucoeff);
   ...
   d.AddDomainIntegrator(new VectorDivergenceIntegrator(one));
   d.Assemble();
   OperatorHandle D;
   d.FormRectangularSystemMatrix(ess_trial_tdof_list, ess_test_tdof_list, D);
   std::unique_ptr<HypreParMatrix> G;

   if (transpose)
   {
      D->MultTranspose(p_tdofs, v_tdofs);
      //vel.Distribute(v_tdofs);
   }
   else
   {
      D->Mult(v_tdofs, p_tdofs);
      //pressure.Distribute(p_tdofs);
   }
