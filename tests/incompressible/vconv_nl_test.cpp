...
int main(int argc, char *argv[])
{
   ...
   ParGridFunction x{&fes}, y{&fes}, y2{&fes};
   x.Randomize(3);
   ParNonlinearForm *nlvc = new ParNonlinearForm(&fes);
   nlvc->SetAssemblyLevel(AssemblyLevel::PARTIAL);
   nlvc->AddDomainIntegrator(new VectorConvectionNLFIntegrator);
   nlvc->Setup();
   x.ParallelProject(x_tdofs);
   nlvc->Mult(x_tdofs, y_tdofs);
   //y2.Distribute(y_tdofs);

   // **************************************************************************
#if 0
   ParNonlinearForm *nl_vconv_fa = new ParNonlinearForm(&fes);
