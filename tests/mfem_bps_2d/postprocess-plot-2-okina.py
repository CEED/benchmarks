...
y_range_gpu=(0,1.2e9) # plot range for the y-axis for GPUs; keep empty for auto
...
#####   Load the data
execfile('postprocess-base-okina.py')
...
devs=list(set([run['device'] for run in sel_runs]))
# print 'Present devices:', devs
dev=devs[0]
print 'Using device:', dev
sel_runs=[run for run in sel_runs if run['device']==dev]
if dev=='gpu':
   y_range=y_range_gpu
...
for plt in pl_set:
   ...
   title('Config: %s %s (%i node%s, %i tasks/node), %s, %s, %s, %s'%(
         code,config_short,num_nodes,'' if num_nodes==1 else 's',
         num_procs_node,compiler,test_short,dev,
   ...
   if write_figures: # write .pdf file?
      pdf_file='plot2_%s_%s-%s_%s_%s_N%03i_pn%i.pdf'%(
               code,test_short,dev,config_short,compiler,num_nodes,
               num_procs_node)
