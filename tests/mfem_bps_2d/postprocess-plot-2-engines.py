...
y_range=(0,1.2e9)     # plot range for the y-axis; comment out for auto
...
#####   Load the data
execfile('postprocess-base-engines.py')
...
for plt in pl_set:
   ...
   for sol_p in sol_p_set:
      ...
      d=[[run['order'],run['num-elem'],1.*run['num-unknowns']/num_procs/vdim,
   ...
   title('Config: %s, host: %s (%i node%s, %i task%s/node), %s, %s'%(
